= Rest Docs API Document
:doctype: book
:icons: font
:source-highlighter: highlightjs
:toc: left
:toclevels: 3
:sectlinks:

[[introduction]]
== Overview

고양이 장난감 가게 API

[[overview]]
== HTTP verbs

HTTP 동사(verbs)는 표준 HTTP와 REST 규약을 따릅니다.

|===
| 동사 | 용례

| `GET`
| 리소스를 가져올 때 사용

| `POST`
| 새 리소스를 만들 때 사용

| `PUT`
| 리소스를 수정할 때 사용

| `PATCH`
| 리소스의 일부를 수정할 때 사용

| `DELETE`
| 리소스를 삭제할 떄 사용
|===

[[overview-http-status-codes]]
== HTTP status code

HTTP 상태 코드는 표준 HTTP와 REST 규약을 따릅니다.

|===
| 상태 코드 | 용례

| `200 OK`
| 요청을 성공적으로 처리함

| `201 Created`
| 새 리소스를 성공적으로 생성함.

| `204 No Content`
| 요청에 대해서 보내줄 수 있는 콘텐츠가 없습니다.

| `400 Bad Request`
| 잘못된 요청을 보낸 경우.
응답 본문에 더 오류에 대한 정보가 담겨있다.

| `403 Forbidden`
| 클라이언트는 콘텐츠에 접근할 권리를 가지고 있지 않습니다.

| `404 Not Found`
| 요청한 리소스가 없음.
|===

[[PRODUCT]]
== PRODUCT

=== GET /products

상품 목록을 JSON 형태로 돌려준다.

==== Request

CURL:

include::{snippets}/get-products/curl-request.adoc[]

Request HTTP Example:
include::{snippets}/get-products/http-request.adoc[]

==== Response

Response Fields:
include::{snippets}/get-products/response-fields.adoc[]

Response HTTP Example:

include::{snippets}/get-products/http-response.adoc[]

=== GET /products/{id}

상품에 대한 자세한 정보를 JSON 형태로 돌려준다.

==== Request

CURL:

include::{snippets}/get-product/curl-request.adoc[]

Request Path Parameters:

include::{snippets}/get-product/path-parameters.adoc[]

Request HTTP Example:
include::{snippets}/get-product/http-request.adoc[]

==== Response

Response Fields:
include::{snippets}/get-product/response-fields.adoc[]

Response HTTP Example:

include::{snippets}/get-product/http-response.adoc[]

=== Create /products

상품을 생성하고 생성 정보를 JSON 형태로 돌려준다.

==== Request

CURL:

include::{snippets}/create-product/curl-request.adoc[]

Request Header:
include::{snippets}/create-product/request-headers.adoc[]

Request Fields:

include::{snippets}/create-product/request-fields.adoc[]

Request HTTP Example:
include::{snippets}/create-product/http-request.adoc[]

==== Response

Response Fields:
include::{snippets}/create-product/response-fields.adoc[]

Response HTTP Example:

include::{snippets}/create-product/http-response.adoc[]

=== Update /products/{id}

상품을 갱신하고 갱신 정보를 JSON 형태로 돌려준다.

==== Request

CURL:

include::{snippets}/update-product/curl-request.adoc[]

Request Header:
include::{snippets}/update-product/request-headers.adoc[]

Request Path Parameters:

include::{snippets}/update-product/path-parameters.adoc[]

Request Fields:

include::{snippets}/update-product/request-fields.adoc[]

Request HTTP Example:
include::{snippets}/update-product/http-request.adoc[]

==== Response

Response Fields:
include::{snippets}/update-product/response-fields.adoc[]

Response HTTP Example:

include::{snippets}/update-product/http-response.adoc[]

=== Delete /products/{id}

상품을 삭제한다.

==== Request

CURL:

include::{snippets}/delete-product/curl-request.adoc[]

Request Header:
include::{snippets}/delete-product/request-headers.adoc[]

Request Path Parameters:

include::{snippets}/delete-product/path-parameters.adoc[]

Request HTTP Example:
include::{snippets}/delete-product/http-request.adoc[]

==== Response

Response HTTP Example:

include::{snippets}/delete-product/http-response.adoc[]

[[USER]]
== USER

=== Create /users

사용자를 생성하고 생성 정보를 JSON 형태로 돌려준다.

==== Request

CURL:

include::{snippets}/create-user/curl-request.adoc[]


Request Fields:

include::{snippets}/create-user/request-fields.adoc[]


Request HTTP Example:
include::{snippets}/create-user/http-request.adoc[]

==== Response

Response Fields:
include::{snippets}/create-user/response-fields.adoc[]

Response HTTP Example:

include::{snippets}/create-user/http-response.adoc[]

=== Update /users/{id}

사용자 정보 갱신하고 갱신 정보를 JSON 형태로 돌려준다.

==== Request

CURL:

include::{snippets}/update-user/curl-request.adoc[]

Request Header:
include::{snippets}/update-user/request-headers.adoc[]

Request Path Parameters:

include::{snippets}/update-user/path-parameters.adoc[]

Request Fields:

include::{snippets}/update-user/request-fields.adoc[]

Request HTTP Example:
include::{snippets}/update-user/http-request.adoc[]

==== Response

Response Fields:
include::{snippets}/update-user/response-fields.adoc[]

Response HTTP Example:

include::{snippets}/update-user/http-response.adoc[]

=== Delete /users/{id}

상품을 삭제한다.

==== Request

CURL:

include::{snippets}/delete-user/curl-request.adoc[]

Request Header:
include::{snippets}/delete-user/request-headers.adoc[]

Request Path Parameters:

include::{snippets}/delete-user/path-parameters.adoc[]

Request HTTP Example:
include::{snippets}/delete-user/http-request.adoc[]

==== Response

Response HTTP Example:

include::{snippets}/delete-user/http-response.adoc[]

[[SESSION]]
== SESSION

=== Create /session

사용자 인증 요청을 처리하고 인증 정보를 리턴한다.

==== Request

CURL:

include::{snippets}/create-session/curl-request.adoc[]


Request Fields:

include::{snippets}/create-session/request-fields.adoc[]


Request HTTP Example:
include::{snippets}/create-session/http-request.adoc[]

==== Response

Response Fields:
include::{snippets}/create-session/response-fields.adoc[]

Response HTTP Example:

include::{snippets}/create-session/http-response.adoc[]
